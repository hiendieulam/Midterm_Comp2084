...
namespace DotNetMidtermA.Controllers
{
    ...
    public class FlightsController : Controller
    {
        ...
        {
            var flights = db.Flights.Include(f => f.FlightType).Include(f => f.Province).OrderBy(f => f.FirstName).ThenBy(f => f.LastName);
        ...
        {
            ViewBag.FlightTypeId = new SelectList(db.FlightTypes.OrderBy(f => f.FirstName), "FlightTypeId", "FlightType1");
            ViewBag.ProvinceId = new SelectList(db.Provinces.OrderBy(f => f.LastName), "ProvinceId", "Name");
            return View();
        }
